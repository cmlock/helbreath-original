...
BOOL CMapData::bGetIsLocateable(short sX, short sY)
{
	...
	dX = sX - m_sPivotX;
	dY = sY - m_sPivotY;
	//Helltrayn 28/05/09. Added this to fix the MIM bug that closes the client
				...
				if ( (m_pData[dX][dY].m_cOwnerAction == DEF_OBJECTATTACK) ) { //3 00498685
					...
					case 6: // Humans
						...
						{	if ((m_pData[dX][dY].m_sAppr2 & 0xF000) != 0) // not Peace mode
							{	if (m_pData[dX][dY].m_sV3 != 1) // other than melee
								{	m_pGame->bAddNewEffect(m_pData[dX][dY].m_sV3, m_sPivotX + dX, m_sPivotY + dY
									...
									if (m_pData[dX][dY].m_sV3 >= 20) m_pGame->PlaySound('E', 43, sDist, lPan); // Sound "missed"
								}
				...
				{	switch (m_pData[dX][dY].m_sOwnerType) {
					...
					case 6:
						if (m_pData[dX][dY].m_cOwnerFrame == 1)
						{
							...
							if (m_pData[dX][dY].m_sV1 == 82) // light if MassMagicMissile around the caster
